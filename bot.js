...
const newDeposit = async (hash, timeStamp, from, to, value) => {
...
var lbank = setInterval(() => {
    (async () => {
        const response = await axios.get(
            "https://api.lbkex.com/v2/ticker/24hr.do?symbol=cra_usdt"
        );
...
function createHorseRace(userId) {
  ...
  */
    ...
    if (userList.get(userId).socketId && userList.get(userId).socketId !== "") {
        io.to(userList.get(userId).socketId).emit(
            "baseprice",
            userList.get(userId).baseprice
        );
    }
    ...
    var horceRace = setInterval(() => {
      ...
      */
            ...
            if (currentPrice.price - userList.get(userId).baseprice > 0) {
                ...
                currentPrice.price - userList.get(userId).baseprice <
                0
            ) {
